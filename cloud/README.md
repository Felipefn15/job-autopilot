...
This is conservative text triage, not a complete semantic classifier. Ambiguous work arrangements can be excluded, and country/work authorization and full resume suitability remain part of the subsequent LLM evaluation. The minimum score applies only after that evaluation. The dashboard explicitly labels unscored jobs “Aguardando análise”. “Fora das preferências” is separate from LLM rejection, with a reason in the job details, and is excluded from the default view.

Saving preferences retriages up to 200 previously stored jobs immediately. Each later batch processes up to 200 additional pending retriage rows. Jobs already submitted, sending/preparing or with uncertain sends are preserved. Previously filtered jobs can return to the analysis queue if preferences change. Changing preferences resets old fit results; passing prefilters does not grant a compatibility score or authorize an application. Existing application evidence is preserved when a pending manual job passes the new prefilters.
