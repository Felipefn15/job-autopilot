...
Authenticated LinkedIn post search runs in the separate [local Chromium collector](../collector/README.md), using a persistent profile on the owner's computer. The computer must stay awake for recurring collection; Cloudflare cannot access its browser session. The cloud receives post text/URLs, not cookies or LinkedIn credentials. No CAPTCHA workarounds are implemented. Imported posts enter the usual evidence-based analysis. Verified email applications use the normal email flow; community posts without verified email require manual follow-up. No live authenticated LinkedIn extraction was verified in this environment; HTML layouts may require selector maintenance or manual import.
